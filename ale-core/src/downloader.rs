...
impl ModelDownloader {
    ...
    pub async fn download_model(&self, model_id: &str) -> Result<PathBuf> {
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| AleError::Other(anyhow::anyhow!("Download error: {}", e)))?;
            ...
            let speed = if elapsed > 0.0 { downloaded as f32 / elapsed } else { 0.0 };
            let remaining_bytes = total_size - downloaded;
            let eta = if speed > 0.0 { (remaining_bytes as f32 / speed) as u32 } else { 0 };
    ...
    pub fn downloaded_models(&self) -> Vec<&ModelInfo> {
        self.known_models.iter()
            .filter(|m| self.is_model_downloaded(&m.id))
            .collect()
    }
    
    /// 获取推荐模型（根据设备性能）
    pub fn recommended_models(&self, device_performance: &crate::inference::DevicePerformance) -> Vec<&ModelInfo> {
        match device_performance {
            crate::inference::DevicePerformance::Low => {
                self.known_models.iter()
                    .filter(|m| m.recommended_for == "低性能设备" || m.recommended_for == "所有设备")
                    .collect()
            }
            crate::inference::DevicePerformance::Medium => {
                self.known_models.iter()
                    .filter(|m| m.recommended_for == "中端设备" || m.recommended_for == "所有设备")
                    .collect()
            }
            crate::inference::DevicePerformance::High => {
                self.known_models.iter()
...
impl ModelDownloadManager {
    ...
    pub async fn download_models_concurrent(&self, model_ids: &[&str]) -> Result<Vec<PathBuf>> {
        ...
        for handle in handles {
            let paths = handle.await.map_err(|e| AleError::Other(anyhow::anyhow!("Task join error: {}", e)))??;
